...
type DeepOmit<T extends object, OuterKey extends _ObjectToKey<T>> = {
  [K in keyof T as K extends OuterKey ? never : K]: T[K] extends object
    ? K extends string // 아래에서 GetMember의 타입 파라미터로 넘기기 위해 반드시 string인지 체크가 필요하다.
      ? T[K] extends Array<infer E>
        ? E extends object
          ? DeepOmit<
              E,
              GetMember<OuterKey, K> extends _ObjectToKey<E, _KeyOfElement<E>>
                ? GetMember<OuterKey, K>
                : never
            >[]
          : T[K]
        : DeepOmit<
            T[K],
            GetMember<OuterKey, K> extends _ObjectToKey<T[K], _KeyOfElement<T[K]>>
              ? GetMember<OuterKey, K>
              : never
          >
      : never
